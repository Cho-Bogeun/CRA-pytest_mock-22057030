...
def test_현재날짜가_일요일인_경우_예약불가_예외처리(customer_with_email, mocker):

    mock_get_now = mocker.patch(
        "booking_scheduler.BookingScheduler.get_now",
        return_value = datetime.strptime("2025/08/03 09:05", "%Y/%m/%d %H:%M")
    )

    boocking_scheduler = BookingScheduler(CAPACITY_PER_HOUR)
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, customer_with_email)

    with pytest.raises(ValueError):
        boocking_scheduler.add_schedule(schedule)

def test_현재날짜가_일요일이_아닌경우_예약가능(mocker, customer_with_email):
    mock_get_now = mocker.patch(
        "booking_scheduler.BookingScheduler.get_now",
        return_value = datetime.strptime("2025/08/04 09:05", "%Y/%m/%d %H:%M")
    )
